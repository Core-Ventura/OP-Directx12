...
void Game::Initialize(::IUnknown* window, int width, int height, DXGI_MODE_ROTATION rotation)
{
    ...
    {
        o.position = XMFLOAT3(RandomNumber(-boundaries.x, boundaries.x), RandomNumber(-boundaries.y, boundaries.y), RandomNumber(-boundaries.z, boundaries.z));
        XMVECTOR randomV = RandomVector() * RandomNumber(0.1f, 0.3f); // Vector de velocidad normalizado random por un valor que lo ralentize y se vea mejor
        XMFLOAT3 fl;
        XMStoreFloat3(&fl, randomV);
        o.velocity = fl;
...
void Game::Update(DX::StepTimer const& timer)
{
    ...
    for (int i = 0; i < numberOfInstances; i++) {
        ...
        XMMATRIX world = XMLoadFloat4x4(&objects.at(i).m_world);
        XMMATRIX rotation = XMMatrixRotationX(delta);

        if (objects.at(i).position.x < -boundaries.x || objects.at(i).position.x > boundaries.x)
        {
            objects.at(i).velocity.x = objects.at(i).velocity.x * -1;
        }

        if (objects.at(i).position.y < -boundaries.y || objects.at(i).position.y > boundaries.y)
        {
            objects.at(i).velocity.y = objects.at(i).velocity.y * -1;
        }

        if (objects.at(i).position.z < -boundaries.z || objects.at(i).position.z > boundaries.z)
        {
            objects.at(i).velocity.z = objects.at(i).velocity.z * -1;
        }

        objects.at(i).position = XMFLOAT3(objects.at(i).position.x   + objects.at(i).velocity.x,
                                          objects.at(i).position.y   + objects.at(i).velocity.y,
                                          objects.at(i).position.z   + objects.at(i).velocity.z);

        XMMATRIX translation = XMMatrixTranslation(objects.at(i).position.x, objects.at(i).position.y, objects.at(i).position.z);
